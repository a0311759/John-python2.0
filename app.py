    ...
    if user_code.strip():
        if write_code_to_file(user_code):
            ...
            if returncode == 0:
                ...
                # Display plots generated by matplotlib and seaborn
                try:
                    exec(user_code)
                    
                    # Check if matplotlib or seaborn plots exist
                    if plt.get_fignums():
                        display_matplotlib_plot()  # Display matplotlib or seaborn plots
                except Exception as plot_error:
                    st.error(f"Plotting error: {plot_error}")
                
                # Plotly plots should be handled separately
                # Since Plotly figures are interactive, they should be displayed using st.plotly_chart
                if 'fig' in locals():
